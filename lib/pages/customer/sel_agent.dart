...
class AgentDetailsPage extends StatefulWidget {
  ...
  const AgentDetailsPage({super.key, required this.agentId, required this.selectedService});
...
class _AgentDetailsPageState extends State<AgentDetailsPage> {
  ...
  Future<AgentDetails?> _fetchAgentDetails() async {
    try {
      final userDocFuture = FirebaseFirestore.instance.collection('users').doc(widget.agentId).get();
      final agentDocFuture = FirebaseFirestore.instance.collection('agents').doc(widget.agentId).get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Agent Profile', style: TextStyle(color: Colors.white)),
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        CircleAvatar(
                          ...
                          child: agent.profilePicUrl.isEmpty ? Text(initial, style: const TextStyle(fontSize: 32, color: darkBlue)) : null,
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(agent.orgName, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: darkBlue)),
                              const SizedBox(height: 4),
                              Row(children: [
                                const Icon(Icons.star, color: Colors.amber, size: 20),
                                const SizedBox(width: 4),
                                Text(agent.rating.toStringAsFixed(1), style: const TextStyle(fontSize: 16, color: grayBlue, fontWeight: FontWeight.bold)),
                              ]),
                            ],
                          ),
                        )
                    ...
                    _buildInfoRow(Icons.location_on_outlined, "${agent.address}, ${agent.city}"),
                    const Divider(height: 32),
                    // Services
                    const Text('All Services Offered', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: darkBlue)),
                    const SizedBox(height: 12),
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: agent.services.map((service) => Chip(
                        label: Text(service),
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text('Send Request for ${widget.selectedService}', style: const TextStyle(color: Colors.white, fontSize: 16)),
                  ),
                ),
              )
            ],
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16),
          Expanded(child: Text(text, style: const TextStyle(fontSize: 16, color: darkBlue))),
        ],
      ),
    );
  }
